...
def update_preview(*args):
    ...
    # ==== DRAW HANDLE (+) ====
    draw.rectangle(
        [quote_pos[0]-handle_size, quote_pos[1]-handle_size,
         quote_pos[0], quote_pos[1]],
        fill="red"
    )

    draw.rectangle(
        [author_pos[0]-handle_size, author_pos[1]-handle_size,
         author_pos[0], author_pos[1]],
        fill="blue"
    )
...
def start_drag(event):
    ...
    # check quote handle
    if (quote_pos[0]-handle_size <= mx <= quote_pos[0] and
        quote_pos[1]-handle_size <= my <= quote_pos[1]):
        drag_target = "quote"

    # check author handle
    elif (author_pos[0]-handle_size <= mx <= author_pos[0] and
          author_pos[1]-handle_size <= my <= author_pos[1]):
        drag_target = "author"
    else:
        drag_target = None
